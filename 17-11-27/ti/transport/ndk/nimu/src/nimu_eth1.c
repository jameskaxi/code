...
)
{
	...
	if (Add_MACAddress (ethInfo, (uint8_t)Cppi_getFlowId(gRxFlowHnd[DNUM])) != 0)
	{
...
static int EmacStop(NETIF_DEVICE* ptr_net_device)
{
    ...
    count = Qmss_getQueueEntryCount(gRxQHnd[DNUM]);
    ...
    for (i=0; i < count; i++) {
         ...
         */
        if ((QMSS_QPOP (gRxQHnd[DNUM], QHANDLER_QPOP_FDQ_ATTACHEDBUF, (Cppi_HostDesc **)&pCppiDesc)) != NULL)
        {
            return -1;
        }
...
static int EmacSend (NETIF_DEVICE* ptr_net_device, PBM_Handle hPkt)
{
    ...
    for (i = 0; i < count; i++)  {
        ...
        QMSS_QPUSH     (gTxFreeQHnd, (Ptr)pHostDesc, pHostDesc->buffLen, NIMU_MAX_DESC_SIZE, Qmss_Location_TAIL);
...
#ifdef PA_LOOPBACK
              QMSS_QPUSH (gPaTxQHnd[0], pCppiDesc, dataBufferSize, NIMU_MAX_DESC_SIZE, Qmss_Location_TAIL);
#else
              Cppi_setPSFlags (Cppi_DescType_HOST, (Cppi_Desc *)pCppiDesc, 0x0);
              /* Send the packet out the mac. It will loop back to PA if the mac/switch
               * have been configured properly
               */
              QMSS_QPUSH (gPaTxQHnd[8], pCppiDesc, dataBufferSize, NIMU_MAX_DESC_SIZE, Qmss_Location_TAIL);
#endif
...
#if DEBUG_TX_DESC
              if (gDescTest == 1) {
                  if ((QMSS_QPOP (gPaTxQHnd[8], QHANDLER_QPOP_FDQ_ATTACHEDBUF, (Cppi_HostDesc **)&pCppiDesc_dbg)) != NULL)
                   {
...
int32_t Setup_Tx (void)
{
	...
	for (i = 0; i < NIMU_NUM_TX_DESC; i++)
	{
		...
		/* Push descriptor to Tx free queue */
		QMSS_QPUSHDESCSIZE (gTxFreeQHnd, pCppiDesc, NIMU_MAX_DESC_SIZE);
	}
...
int32_t Setup_Rx (NETIF_DEVICE*     ptr_net_device)
{
	...
	ListAddress = l2_global_address((uint32_t) &gEthHiPriAccumList[DNUM][0]);

	if ((gRxQHnd[DNUM] = Qmss_queueOpen (Qmss_QueueType_HIGH_PRIORITY_QUEUE, QMSS_HIGH_PRIORITY_QUEUE_BASE+DNUM, &isAllocated)) < 0)
	{
	...
	if(Qmss_configureAccTimer (Qmss_PdspId_PDSP1, 1750) != QMSS_ACC_SOK)
	...
	for (i = 0; i < NIMU_NUM_RX_DESC; i++)
	{
		...
		QMSS_QPUSHDESCSIZE (gRxFreeQHnd, pCppiDesc, NIMU_MAX_DESC_SIZE);
	}
	...
	rxQInfo = Qmss_getQueueNumber (gRxQHnd[DNUM]);
	...
	if ((gRxFlowHnd[DNUM] = Cppi_configureRxFlow (res_mgr_cppi_get_passhandle(), &rxFlowCfg, &isAllocated)) == NULL)
	{
...
int32_t Add_MACAddress (paEthInfo_t ethInfo, uint8_t flowId)
{
    ...
    if ((QMSS_QPOP (gTxFreeQHnd, QHANDLER_QPOP_FDQ_NO_ATTACHEDBUF, (Cppi_HostDesc **)&pHostDesc )) != NULL)
    {
    ...
    for (j = 0; j < 100; j++)
    {
        ...
        if (Qmss_getQueueEntryCount (gPaCfgCmdRespQHnd) > 0)
        {
            ...
            if (pHostDesc->softwareInfo0 != cmdReplyInfo.replyId)
            {
                ...
                QMSS_QPUSH (gTxFreeQHnd, pHostDesc, pHostDesc->buffLen, NIMU_MAX_DESC_SIZE, Qmss_Location_TAIL);
                ret_val = -1;
                goto return_fail;
            ...
            pHostDesc->buffLen = pHostDesc->origBufferLen;
            QMSS_QPUSH (gRxFreeQHnd, pHostDesc, pHostDesc->buffLen, NIMU_MAX_DESC_SIZE, Qmss_Location_TAIL);
            break;
        }
    }
...
void EmacRxPktISR(NETIF_DEVICE* ptr_net_device)
{
    ...
    if(
        ((uint32_t)(&gEthHiPriAccumList[DNUM][0]) & EMAC_EXTMEM ) ||
        ((uint32_t)(&gEthHiPriAccumList[DNUM][0]) & EMAC_MSMCSRAM )
      )
    {
        /* This needs to be enabled if gEthHiPriAccumList is in External Memory or MSMCMEM */
        /* Inv cache based on the qhandler type*/
        CACHE_invL1d((void *)&gEthHiPriAccumList[DNUM][0], sizeof(gEthHiPriAccumList)/8, CACHE_WAIT);
        CACHE_invL2((void *) &gEthHiPriAccumList[DNUM][0], sizeof(gEthHiPriAccumList)/8, CACHE_WAIT);
    }

    //i           = MAX_HI_PRI_ACCUM_LIST_SIZE - 1 - (RX_INT_THRESHOLD+1);
    ListAddress = (uint32_t* )l2_global_address((uint32_t) &gEthHiPriAccumList[DNUM][0]);
    ...
    for (i = 1; i <= count; i ++)
    {
        ...
        if (NULL == (hPkt = PBM_alloc(1514))) {
            /* End Critical Section */
            Osal_qmssCsExit (key);            /* Clear INTD */
            Hwi_restoreInterrupt(ETH_INTERRUPT, coreKey [CSL_chipReadReg (CSL_CHIP_DNUM)]);
            Qmss_ackInterrupt(PA_ACC_CHANNEL_NUM, 1);
            Qmss_setEoiVector(Qmss_IntdInterruptType_HIGH, PA_ACC_CHANNEL_NUM);
          return; // Allocation in the Rx ISR failed
        }
        ...
        */
        ...
        gTime = CSL_tscRead();
//        edma_config[0].myParamSetup.srcAddr    = (Uint32)pBuffer;
//        edma_config[0].myParamSetup.aCntbCnt   = CSL_EDMA3_CNT_MAKE(pktLen,1);
//        edma_config[0].myParamSetup.dstAddr    = (Uint32)rx_pbm_pkt->pDataBuffer;
//        edma_config[0].myParamSetup.srcDstBidx = CSL_EDMA3_BIDX_MAKE(1,1);
//        edma_config[0].myParamSetup.linkBcntrld= CSL_EDMA3_LINKBCNTRLD_MAKE(0,0);
//        edma_config[0].myParamSetup.srcDstCidx = CSL_EDMA3_CIDX_MAKE(1,1);
//        edma_config[0].myParamSetup.cCnt = 1;
//        edma_setParam(&edma_config[0]);
//        edma_trigger(&edma_config[0]);
//        CACHE_invL1d((void *)rx_pbm_pkt->pDataBuffer, pktLen, CACHE_WAIT);
//        CACHE_invL2((void *) rx_pbm_pkt->pDataBuffer, pktLen, CACHE_WAIT);
       ...
       /* Free the packet back to the Rx FDQ */
        pHostDesc->buffLen = pHostDesc->origBufferLen;
        QMSS_QPUSH (gRxFreeQHnd, (Ptr)pHostDesc, pHostDesc->buffLen, NIMU_MAX_DESC_SIZE, Qmss_Location_TAIL);
    }
     ...
     if (!gIsPingListUsed)
    {
        ...
        i = sizeof (gEthHiPriAccumList)/16;
