...
//	TSCL = 0;
    ...
    Config_Cpsw_Pa_Bypass();
        ...
        memset ((void *)&gNimuHostDesc, 0, sizeof(gNimuHostDesc));
        Cache_wbInv ((void *) gNimuHostDesc, sizeof(gNimuHostDesc), Cache_Type_L1D, CACHE_WAIT);
        Cache_wbInv ((void *) gNimuHostDesc, sizeof(gNimuHostDesc), Cache_Type_L2, CACHE_WAIT);
        /* Memory Region Configuration */
        memRegInfo.descBase         = (uint32_t *)l2_global_address((uint32_t)gNimuHostDesc);
        ...
        if ((gEthFreeQHnd = Cppi_initDescriptor (&cppiDescCfg, &numAllocated)) <= 0)
        {
        ...
        memset (gIPCDesc, 0, sizeof(gIPCDesc) );
        Cache_wbInv ((void *) gIPCDesc, sizeof(gIPCDesc), Cache_Type_L1D, CACHE_WAIT);
        Cache_wbInv ((void *) gIPCDesc, sizeof(gIPCDesc), Cache_Type_L2, CACHE_WAIT);
        memRegInfo.descBase         = (uint32_t *)l2_global_address((uint32_t)gIPCDesc);
        ...
        for(i=0; i<8; i++)
        {
            gIPCQHnd[i] = Qmss_queueOpen (Qmss_QueueType_GENERAL_PURPOSE_QUEUE, QMSS_PARAM_NOT_SPECIFIED, &isAllocated);
            gIPCQHndPoll[i] = Qmss_queueOpen (Qmss_QueueType_GENERAL_PURPOSE_QUEUE, QMSS_PARAM_NOT_SPECIFIED, &isAllocated);
        }
        ...
        if ((gIPCFreeQHnd  = Cppi_initDescriptor (&cppiDescCfg, &numAllocated)) < 0)
        {
